...
class Addmore extends React.Component {
  ...
  render() {
    const { onChange, Item, children, remover, ...restProps } = this.props;
    ...
    const addItem = () => {
      const newDataArr = [...dataArr, { _key: dataArr.length }];
      triggerChange(newDataArr);
    };
                ...
                <div key={d._key} style={{ display: "flex", padding: "8px" }}>
                  <div style={{ flex: 1 }}>
                    <Item
                      {...restProps}
                      value={d}
                      onChange={value => itemChange(i, value)}
                    />
                  </div>
                  <div style={{ flex: 0 }}>
